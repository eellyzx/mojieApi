 ...
 * @LastEditTime: 2020-07-03 17:46:39
 * @Description: 
 */
...
use Redis\Connections\Connection;
use Closure;
use Redis;
use RedisCluster;
use PhpHelper;
...
class PhpRedisConnection extends Connection
{
    ...
    public function pipeline(callable $callback = null)
    {
        ...
        return is_null($callback)
            ? $pipeline
            : PhpHelper::tap($pipeline, $callback)->exec();
    }
    ...
    public function transaction(callable $callback = null)
    {
        ...
        return is_null($callback)
            ? $transaction
            : PhpHelper::tap($transaction, $callback)->exec();
    }
    ...
    {   
        ...
        try {
            $result = 0 === strpos($result, 'serialize:') ? unserialize(substr($result, 10)) : $result;
        } catch (\Exception $e) {
            $result = false;
        }
    ...
    public function setex($key, $timeout, $value)
    {
        ...
        return (int) $this->command('setex', [$key, $timeout, $value]);
    }
    ...
    public function flushdb()
    {
        ...
        foreach ($this->client->_masters() as [$host, $port]) {
            (new Redis)->connect($host, $port)->flushDb();
        }
    }
